...
int add_new_passenger() {
	...
	scanf_s("%u", &new_passenger.origin.street);

	printf("Enter the Orig avenue -> ");
	scanf_s("%u", &new_passenger.origin.avenue);

	printf("Enter the Orig puc -> ");
	scanf_s("%u", &new_passenger.pick_up_corner);

	printf("Enter the dest street -> ");
	scanf_s("%u", &new_passenger.destination.street);

	printf("Enter the dest avenue -> ");
	scanf_s("%u", &new_passenger.destination.avenue);

	printf("Enter the drop-of-corner -> ");
	scanf_s("%u", &new_passenger.drop_off_corner);
	...
	/* display the new passenger record*/
	printf("\npassenger ID: %-6u Request time: %-8u Orig street: %-8u Orig avenue: %u\n",
		new_passenger.pid, new_passenger.time_req, new_passenger.origin.street, new_passenger.origin.avenue);
	printf("passenger ID: %-6u orig puc: %-12u dest street: %-8u dest avenue: %u\n",
		new_passenger.pid, new_passenger.pick_up_corner, new_passenger.destination.street, new_passenger.destination.avenue);
	printf("passenger ID: %-6u drop-of-corner: %-6u passenger state: %u\n\n\n",
		new_passenger.pid, new_passenger.drop_off_corner, new_passenger.pass_state);
	...
	/* update the header file to contain to hold the next available passenger id*/
