...
int init_passenger(void) {
	...
	*/
	...
	while (fgets(passenger_rec, LENGTH, (FILE*)pfdi) > 0) {
		...
		pass_struct.origin.street = str_2_num(pass_tok);

		pass_tok = strtok(NULL, "\t");
		pass_struct.origin.avenue = str_2_num(pass_tok);

		pass_tok = strtok(NULL, "\t");
		pass_struct.pick_up_corner = str_2_num(pass_tok);

		pass_tok = strtok(NULL, "\t");
		pass_struct.destination.street = str_2_num(pass_tok);

		pass_tok = strtok(NULL, "\t");
		pass_struct.destination.avenue = str_2_num(pass_tok);

		pass_tok = strtok(NULL, "\t");
		pass_struct.drop_off_corner = str_2_num(pass_tok);
	...
	header_pass_rec.first_id = passenger_id;
